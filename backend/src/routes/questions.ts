...
// Create question
router.post('/', authenticate, requireRole(['school', 'tutor']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    const { examId, questionText, questionType, options, correctAnswer, marks, questionOrder, imageUrl } = req.body;
    const user = req.user!;
    ...
    const result = await client.query(
      `INSERT INTO questions (exam_id, question_text, question_type, options, correct_answer, marks, question_order, image_url)
       ...
       RETURNING *`,
      [examId, questionText, questionType, JSON.stringify(options), correctAnswer, marks, questionOrder, imageUrl]
    );
...
router.post('/bulk', authenticate, requireRole(['school', 'tutor']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      const result = await client.query(
        `INSERT INTO questions (exam_id, question_text, question_type, options, correct_answer, marks, question_order)
         ...
         RETURNING *`,
        [examId, q.questionText, q.questionType, JSON.stringify(q.options), q.correctAnswer, q.marks, q.questionOrder || i + 1]
      );
...
// Update question
router.put('/:id', authenticate, requireRole(['school', 'tutor']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    const { id } = req.params;
    const { questionText, questionType, options, correctAnswer, marks, questionOrder, imageUrl } = req.body;
    const user = req.user!;
    ...
    const result = await client.query(
      `UPDATE questions 
       ...
       WHERE id = $8
       RETURNING *`,
      [questionText, questionType, options ? JSON.stringify(options) : null, correctAnswer, marks, questionOrder, imageUrl, id]
    );
...
router.delete('/:id', authenticate, requireRole(['school', 'tutor']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    const { id } = req.params;
    const user = req.user!;
...
router.post('/ai-generate', authenticate, requireRole(['school', 'tutor']), async (req: Request, res: Response) => {
  try {
    const { topic, subject, numQuestions, difficulty, questionType } = req.body;

    if (!process.env.OPENAI_API_KEY) {
      return res.status(500).json({ success: false, message: 'AI generation not configured' });
    }
    ...
    const completion = await openai.chat.completions.create({
      model: 'gpt-3.5-turbo',
      messages: [
        { role: 'system', content: 'You are a helpful educational assistant that generates exam questions.' },
    ...
    const formattedQuestions = generatedQuestions.map((q: any, index: number) => ({
      ...
      options: q.options || (questionType === 'true_false' ? ['True', 'False'] : []),
...
// Reorder questions
router.post('/reorder', authenticate, requireRole(['school', 'tutor']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
