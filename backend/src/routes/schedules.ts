...
router.get('/available-students', authenticate, requireRole(['school', 'tutor']), async (req: Request, res: Response) => {
      ...
      try {
        ...
        const result = await client.query(
          `INSERT INTO exam_schedules (exam_id, student_id, scheduled_date, start_time, end_time, access_code, status, created_by)
           ...
           RETURNING *`,
          [examId, studentId, scheduledDate, startTime, endTime, accessCode, user.id]
        );
...
router.get('/student/my-exams', authenticate, requireRole(['student']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    const user = req.user!;
...
router.post('/verify-access', authenticate, requireRole(['student']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
