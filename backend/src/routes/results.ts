...
router.post('/submit', authenticate, requireRole(['student']), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const percentage = schedule.total_marks > 0 ? (totalScore / schedule.total_marks) * 100 : 0;
...
router.get('/my-result/:scheduleId', authenticate, requireRole(['student']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
...
router.get('/exam/:examId', authenticate, requireRole(['school', 'tutor']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
...
router.get('/my-history', authenticate, requireRole(['student']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    const user = req.user!;
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch exam history' });
  } finally {
    client.release();
  }
});
...
router.post('/grade-theory', authenticate, requireRole(['school', 'tutor']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    ...
    if (examCheck.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Student exam not found' });
    }
    ...
    for (const grade of grades) {
      const answerIndex = answers.findIndex((a: any) => a.questionId === grade.questionId);
      if (answerIndex !== -1) {
    ...
    totalScore = answers.reduce((sum: number, a: any) => sum + (a.marksObtained || 0), 0);
    const percentage = studentExam.total_marks > 0 ? (totalScore / studentExam.total_marks) * 100 : 0;
    ...
    await client.query(
      ...
      [
        ...
        percentage >= studentExam.pass_mark_percentage ? 'completed' : 'failed',
        studentExamId
      ]
    );
    ...
    res.status(500).json({ success: false, message: 'Failed to grade theory questions' });
  } finally {
    client.release();
  }
});
...
// Get exam statistics
router.get('/exam/:examId/statistics', authenticate, requireRole(['school', 'tutor']), async (req: Request, res: Response) => {
  const client = await pool.connect();
  try {
    const { examId } = req.params;
    const user = req.user!;
