...
// Excel parsing was removed due to security advisory in the `xlsx` package.
// Use CSV uploads instead. If Excel import is required in the future,
// implement server-side parsing with a safe library (e.g., `exceljs`).

// Validate tutor CSV data
export function validateTutorCSV(
  data: TutorCSVRow[],
): ParseResult<TutorCSVRow> {
  const errors: string[] = [];
...
export function downloadTemplate(
  type: "tutors" | "students" | "questions",
): void {
  let headers: string[] = [];
...
// Excel export was removed due to security advisory in the `xlsx` package.
// Use CSV export via `exportToCSV` instead for now.
